...
def add_model_config_args(parser):
    ...
    # VQ-VAE Quantizer
    group.add_argument('--quantizer', type=str, default="ema", choices=['ema','origin','fsq','lfq','rfsq'],
    ...
    group.add_argument('--codebook-loss-weight', type=float, default=0.)
        #RFSQ
    group.add_argument('--rfsq-stages', type=int, default=3,
                        help="number of RFSQ stages")
    group.add_argument('--rfsq-strategy', type=str, default='scale', choices=['scale', 'layernorm', 'none'],
                       help="RFSQ strategy: scale, layernorm, or none")
    group.add_argument('--rfsq-initial-scale', type=float, default=1.0,
                        help="initial scale factor for RFSQ")
...
def add_data_args(parser):
    ...
    group.add_argument('--train-data-path', type=str, default="./data/imagenet/train",
                        help="the path of training data")
    group.add_argument('--val-data-path', type=str, default="./data/imagenet/val", 
...
def get_args():
    ...
    if args.quantizer == 'lfq':
        # 对于LFQ，如果没有明确指定embed_dim，则使用lfq_dim
        if args.embed_dim == 256:  # 默认值，说明没有明确指定
        ...
        args.embed_dim = len(args.levels)
        args.save += '-rfsq-stages-' + str(args.rfsq_stages)
        args.save += '-strategy-' + str(args.rfsq_strategy)
        args.save += '-n_embed-' + str(multiplyList(args.levels))

    elif args.quantizer == 'ema':
        # VQ-EMA的embed_dim保持用户指定值
        args.save += '-n_embed-' + str(args.n_embed)
        args.save += '-embed_dim-' + str(args.embed_dim)
    else:
        args.save += '-n_embed-' + str(args.n_embed)
    return args
