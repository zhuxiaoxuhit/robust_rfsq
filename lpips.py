...
class vgg16(torch.nn.Module):
    def __init__(self, requires_grad=False, pretrained=True):
        ...
        if pretrained:
            import os
            local_vgg_path = "models/pretrained/vgg16.pth"
            
            if os.path.exists(local_vgg_path):
                # 从本地加载VGG16权重
                vgg_model = models.vgg16(pretrained=False)  # 不从网络下载
                vgg_model.load_state_dict(torch.load(local_vgg_path, map_location='cpu'))
                vgg_pretrained_features = vgg_model.features
                print(f"✅ 从本地加载VGG16模型: {local_vgg_path}")
            else:
                # 备选方案：从网络下载
                print(f"⚠️  本地VGG模型不存在 ({local_vgg_path})，从网络下载...")
